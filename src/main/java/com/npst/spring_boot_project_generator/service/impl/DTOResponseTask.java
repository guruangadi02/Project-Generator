...
@Slf4j
@Component
public class DTOResponseTask implements Task {
    ...
    public void execute(ResourceModel resourceModel) {
        ...
        log.info("✅ Creating DTO Response Layer Files...");

        final String name = AppConstants.DTO_FOLDER + "." + AppConstants.DTO_RESPONSE_FOLDER;
        final String file = Utility.capitalize(resourceModel.getResource()) + "Response" + "DTO";

        final String CLASS_NAME_DTO = file.toString();
        final String BASE_PACKAGE = resourceModel.getAdditionalProperty(AppConstants.BASE_PACKAGE_KEY).toString();
        final String BASE_PACKAGE_DTO = BASE_PACKAGE + "." + name;
        ...
        Optional<Map<String, Object>> stringObjectMap = Optional.ofNullable(resourceModel.getRequests())
                ...
                .flatMap(list -> list.stream()
                        .filter(req -> Utility.isPostOrGet(req.getHttpMethod()))
                        .findFirst()
                        .flatMap(req -> Utility.getFirstHttpResponse(req))
                );

        Map<String, Object> stringObjMap = stringObjectMap.orElse(Map.of());
        ...
        stringObjectMap.ifPresentOrElse(
                reqestData -> {
                    ...
                    DTOWithJson dtoWithJson = nestedEntityTask.buildNestedDTO(CLASS_NAME_DTO, reqestData, false);

                    // Create the Java file
                    JavaFile javaFile = JavaFile.builder(BASE_PACKAGE_DTO, dtoWithJson.getTypeSpec()).build();
                    ...
                    String entityFilePath = PROJECT_PATH +
                            BASE_PACKAGE_DTO.replace(".", "/") + "/" +
                            CLASS_NAME_DTO + ".java";

                    resourceModel.setAdditionalProperty(AppConstants.DTO_RESPONSE_PATH, entityFilePath);
                    resourceModel.setAdditionalProperty(resourceModel.getResource(), dtoWithJson.getJson());

                    modelFactory.updateApiResponseModel(ApiResponseModel.DTO, entityFilePath);


                    log.info("✅ Spring Boot DTO Response File generated successfully at: {}", entityFilePath);

                },
                () -> log.warn("No Request found for DTO")
        );
